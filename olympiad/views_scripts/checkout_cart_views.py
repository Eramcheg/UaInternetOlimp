...
from olympiad.decorators import login_required_or_session, logout_required
from olympiad.views import db, orders_ref, serialize_firestore_document, itemsRef, get_cart, cart_ref, single_order_ref, \
...
from olympiad.forms import UserRegisterForm, User
import os
import json
import time
import logging
...
from olympiad.views_scripts.auth_views import get_new_user_id
from olympiad.views_scripts.profile_views import get_user_addresses
...
def cart_page(request):
    ...
    return render(request, 'oliverweber/cart.html', context=context)
...
def anonym_cart_info(request):
    ...
    return render(request, 'oliverweber/orderAnonymously.html', context=context)
        ...
        if form.is_valid():
            ...
            if user is not None:
                ...
                if firebase_user_doc and firebase_user_doc[0].to_dict().get('Enabled', True) == False:
                    ...
                    return render(request, 'oliverweber/orderAnonymously.html', {'form': form})
                else:
...
def checkout_addresses(request):
    ...
    return render(request, 'oliverweber/orderAnonymAddresses.html', context=context)